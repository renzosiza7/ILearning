<template>
  <div>
    ...
    <div class="row mt-3">      
      <div class="col-lg-6">
          <h3 class="text-dark">Soluciones</h3>
      </div>
      <div class="col-lg-6">
          <button class="btn btn-success float-right" @click="create">
                <i class="fa fa-plus"></i>&nbsp;Nueva Solución
          </button>
      </div>
    </div>
    <hr class="bg-light">  
  ...
  export default {   
    ...
    methods: {              
        fetch() {
          ...
          axios.get(`${this.url}/errors/${this.error.id}/solutions`).then(function (response) {              
            me.$vs.loading.close()
            me.solutions = response.data;
